...
class SaveMedicationUseCase @Inject constructor(private val repository: MedicationRepository) {
    suspend operator fun invoke(name: String, grammage: String, optional: Boolean, dosage: Float = -1f, numberOfPills: Float = -1f): Boolean{
        val medication = Medication(null, name, grammage, dosage, optional, numberOfPills)
