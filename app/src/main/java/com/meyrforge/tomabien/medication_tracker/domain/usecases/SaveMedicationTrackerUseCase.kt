...
class SaveMedicationTrackerUseCase @Inject constructor(private val repository: MedicationTrackerRepository) {
    suspend operator fun invoke(medId: Int, date: String, hour: String){
        val tracker = MedicationTracker(null, date, hour, medId)
        repository.saveMedicationTracker(tracker)
    }
}